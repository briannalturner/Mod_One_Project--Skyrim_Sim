...
### SOLITUDE NPCS ###
dervenin = Npc.find_or_create_by(name: "Dervenin", description: "Dervenin is sitting on a ragged mat begging passersby for septims.")
...
### INTERACTIONS ###
### markarth ###
kill_vorstag = Interaction.find_or_create_by(description: "You slice Vorstag cleanly through the chest with your blade.", npc_id: vorstag.id, bounty: 1000, town_id: markarth.id)
talk_vorstag = Interaction.find_or_create_by(description: "Some call me a mercenary, but I like to think of myself as a freelance adventurer for hire.", npc_id: vorstag.id, town_id: markarth.id)
kill_markarth_jarl = Interaction.find_or_create_by(description: "You destroy Jarl Igmund's torso with a firebolt spell.", npc_id: markarth_jarl.id, bounty: 1000, town_id: markarth.id)
talk_markarth_jarl = Interaction.find_or_create_by(description: "Are you a sellsword? I don't like your kind. No honor. No loyalty. Unreliable. But I can't turn away a potential ally in these times.", npc_id: markarth_jarl.id, town_id: markarth.id)
kill_calcelmo = Interaction.find_or_create_by(description: "You cleave Calcelmo's head in two with your sword.", npc_id: calcelmo.id, bounty: 1000, town_id: markarth.id)
talk_calcelmo = Interaction.find_or_create_by(description: "You idiot. Do you even know who I am? The most recognized scholar on the Dwemer in all of Tamriel, and you people keep bothering me! I... I'm sorry I... I got too excited. I'm in the middle of some very... stressful work, and I shouldn't have yelled. How can I help you?", npc_id: calcelmo.id, town_id: markarth.id)

### dawnstar ###
kill_wayfinder = Interaction.find_or_create_by(description: "You slice Captain Wayfinder cleanly through the chest with your blade.", npc_id: capt_wayfinder.id, bounty: 1000, town_id: dawnstar.id)
talk_wayfinder = Interaction.find_or_create_by(description: "Everyone thinks I'm too young to be a ship's captain, but I'm going to prove them wrong. I don't buy all that talk about ship captains having to be stern and strict. I want my crewmen to enjoy their work.", npc_id: capt_wayfinder.id, town_id: dawnstar.id)
kill_dawnstar_jarl = Interaction.find_or_create_by(description: "You destroy Jarl Skald the Elder's torso with a firebolt spell.", npc_id: dawnstar_jarl.id, bounty: 1000, town_id: dawnstar.id)
talk_dawnstar_jarl = Interaction.find_or_create_by(description: "Are you a sellsword? I don't like your kind. No honor. No loyalty. Unreliable. But I can't turn away a potential ally in these times.", npc_id: dawnstar_jarl.id, town_id: dawnstar.id)
kill_alesan = Interaction.find_or_create_by(description: "You seriously tried to kill a kid? You're sick.", npc_id: alesan.id, bounty: 1000, town_id: dawnstar.id)
talk_alesan = Interaction.find_or_create_by(description: "My parents are dead. My pa was a sailor. Took me with him on his last trip. After he got sick, they... put us off here and left without us. He... died a few days later. I've tried to make do since then, but... it's been hard. Real hard.", npc_id: alesan.id, town_id: dawnstar.id)

    
### morthal ###
kill_lami = Interaction.find_or_create_by(description: "You slice Lami cleanly through the chest with your blade.", npc_id: lami.id, bounty: 1000, town_id: morthal.id)
talk_lami = Interaction.find_or_create_by(description: "If you need any alchemy supplies, I'll do my best to provide them. Assuming I haven't used them all up.", npc_id: lami.id, town_id: morthal.id)
kill_morthal_jarl = Interaction.find_or_create_by(description: "You destroy Jarl Idgrod Ravencrone's torso with a firebolt spell.", npc_id: morthal_jarl.id, bounty: 1000, town_id: morthal.id)
talk_morthal_jarl = Interaction.find_or_create_by(description: "So, life has brought you to Morthal, and to me. What purpose this serves, we will no doubt see. Welcome.", npc_id: morthal_jarl.id, town_id: morthal.id)
kill_lurbuk = Interaction.find_or_create_by(description: "You cleave Lurbuk's head in two with your sword.", npc_id: lurbuk.id, bounty: 1000, town_id: morthal.id)
talk_lurbuk = Interaction.find_or_create_by(description: "Don't fret over the gloom, there's nothing to fear! Old Lurbuk will brighten your spirits -- just lend him your ear! Try as you may, they won't say 'good day,' but will just walk away. Worry not, Morthal! Lurbuk is here to stay!", npc_id: lurbuk.id, town_id: morthal.id)


### solitude ###
kill_dervenin = Interaction.find_or_create_by(description: "You slice Dervenin cleanly through the chest with your blade.", npc_id: dervenin.id, bounty: 1000, town_id: solitude.id)
talk_dervenin = Interaction.find_or_create_by(description: "Tendrils of thought may wind their way through this realm, but those tendrils bind our reality together. A method to your madness and your madness is the method and the melody...", npc_id: dervenin.id, town_id: solitude.id)
kill_solitude_jarl = Interaction.find_or_create_by(description: "You destroy Jarl Elisif the Fair's torso with a firebolt spell.", npc_id: solitude_jarl.id, bounty: 1000, town_id: solitude.id)
talk_solitude_jarl = Interaction.find_or_create_by(description: "With the passing of my dear husband, Torygg, the weighty position of Jarl has fallen to me. Please direct questions to my steward.", npc_id: solitude_jarl.id, town_id: solitude.id)
kill_svari = Interaction.find_or_create_by(description: "You seriously tried to kill a kid? You're sick.", npc_id: svari.id, bounty: 1000, town_id: solitude.id)
talk_svari = Interaction.find_or_create_by(description: "Mum doesn't say much since Uncle Roggvir died. She doesn't even go to Temple anymore. I wish she would. At least that way she'd have people to talk to.", npc_id: svari.id, town_id: solitude.id)


### guard interactions ###
kill_markarth_guard = Interaction.find_or_create_by(description: "You eviscerate the Guard's neck with one swipe of your sword.", npc_id: markarth_guard.id, bounty: 1000, town_id: markarth.id)
kill_dawnstar_guard = Interaction.find_or_create_by(description: "You eviscerate the Guard's neck with one swipe of your sword.", npc_id: dawnstar_guard.id, bounty: 1000, town_id: dawnstar.id)
kill_morthal_guard = Interaction.find_or_create_by(description: "You eviscerate the Guard's neck with one swipe of your sword.", npc_id: morthal_guard.id, bounty: 1000, town_id: morthal.id)
kill_windhelm_guard = Interaction.find_or_create_by(description: "You eviscerate the Guard's neck with one swipe of your sword.", npc_id: windhelm_guard.id, bounty: 1000, town_id: windhelm.id)
kill_falkreath_guard = Interaction.find_or_create_by(description: "You eviscerate the Guard's neck with one swipe of your sword.", npc_id: falkreath_guard.id, bounty: 1000, town_id: falkreath.id)
kill_solitude_guard = Interaction.find_or_create_by(description: "You eviscerate the Guard's neck with one swipe of your sword.", npc_id: solitude_guard.id, bounty: 1000, town_id: solitude.id)
kill_riften_guard = Interaction.find_or_create_by(description: "You eviscerate the Guard's neck with one swipe of your sword.", npc_id: riften_guard.id, bounty: 1000, town_id: riften.id)
kill_whiterun_guard = Interaction.find_or_create_by(description: "You eviscerate the Guard's neck with one swipe of your sword.", npc_id: whiterun_guard.id, bounty: 1000, town_id: whiterun.id)
kill_winterhold_guard = Interaction.find_or_create_by(description: "You eviscerate the Guard's neck with one swipe of your sword.", npc_id: winterhold_guard.id, bounty: 1000, town_id: winterhold.id)
talk_markarth_guard = Interaction.find_or_create_by(description: "I used to be an adventurer like you, then I took an arrow in the knee.", npc_id: markarth_guard.id, town_id: markarth.id)
talk_dawnstar_guard = Interaction.find_or_create_by(description: "I used to be an adventurer like you, then I took an arrow in the knee.", npc_id: dawnstar_guard.id, town_id: dawnstar.id)
talk_morthal_guard = Interaction.find_or_create_by(description: "I used to be an adventurer like you, then I took an arrow in the knee.", npc_id: morthal_guard.id, town_id: morthal.id)
talk_windhelm_guard = Interaction.find_or_create_by(description: "I used to be an adventurer like you, then I took an arrow in the knee.", npc_id: windhelm_guard.id, town_id: windhelm.id)
talk_falkreath_guard = Interaction.find_or_create_by(description: "Let me guess... someone stole your sweetroll.", npc_id: falkreath_guard.id, town_id: falkreath.id)
talk_solitude_guard = Interaction.find_or_create_by(description: "Let me guess... someone stole your sweetroll.", npc_id: solitude_guard.id, town_id: solitude.id)
talk_riften_guard = Interaction.find_or_create_by(description: "Let me guess... someone stole your sweetroll.", npc_id: riften_guard.id, town_id: riften.id)
talk_whiterun_guard = Interaction.find_or_create_by(description: "Let me guess... someone stole your sweetroll.", npc_id: whiterun_guard.id, town_id: whiterun.id)
talk_winterhold_guard = Interaction.find_or_create_by(description: "Let me guess... someone stole your sweetroll.", npc_id: winterhold_guard.id, town_id: winterhold.id)

### Riften Interactions ###
kill_edda = Interaction.find_or_create_by(description: "You straight up kill her.", npc_id: edda.id, bounty: 0, goodwill: 0, town_id: riften.id)
support_edda = Interaction.find_or_create_by(description: "You hand Edda 5 gold.", npc_id: edda.id, bounty: 0, goodwill: 100, town_id: riften.id)
help_maven = Interaction.find_or_create_by(description: "You help Maven, by finding into the double-agent stealing all of her mead!", bounty: 0, goodwill: 100, npc_id: maven, town_id: riften.id)
kill_maven = Interaction.find_or_create_by(description: "You assasinate Maven Black Briar... you cause havoc in all of Riften", bounty: 1000, goodwill: 0, npc_id: maven, town_id: riften.id)
kill_riften_guard = Interaction.find_or_create_by(description: "You assasinate the guard when no one is watching... you still get a bounty though.", npc_id: riften_guard, bounty: 0, goodwill: 100, town_id: riften.id)
talk_riften_guard = Interaction.find_or_create_by(description: "Watch the Skies traveller", npc_id: riften_guard, bounty: 0, goodwill: 100, town_id: riften.id)
### Windhelm Interactions ###
talk_windhelm_guard = Interaction.find_or_create_by(description: "Welcome to Windhelm traveller.", npc_id: windhelm_guard.id, bounty: 0, goodwill: 0, town_id: windhelm.id)
kill_windhelm_guard = Interaction.find_or_create_by(description: "You eviscerate the Guard's neck with one swipe of your sword.", npc_id: windhelm_guard.id, bounty: 100, goodwill: 0, town_id: windhelm.id)
help_aventus = Interaction.find_or_create_by(description: "You kill the cruel matron, Grelod the Kind, who abused Aventus and the other children.", goodwill: 100, town_id: windhelm.id)
help_jarl = Interaction.find_or_create_by(description: "You learn of a murderer on the loose, and you eventually capture him.", goodwill: 100, bounty: 0, town_id: windhelm.id)
kill_jarl = Interaction.find_or_create_by(description: "You kill Ulfric with your Shout... Karma", goodwill: 0, bounty: 1000, town_id: windhelm.id)

### Winterhold Interactions ###
kill_savos_aren = Interaction.find_or_create_by(description: "You slice Savos Aren cleanly through the chest with your blade.", npc_id: savos_aren.id, bounty: 1000, town_id: winterhold.id)
...
kill_winterhold_jarl = Interaction.find_or_create_by(description: "You destroy Jarl Korir's torso with a firebolt spell.", npc_id: winterhold_jarl.id, bounty: 1000, town_id: winterhold.id)
...
kill_ancano = Interaction.find_or_create_by(description: "You cleave Lurbuk's head in two with your sword.", npc_id: ancano.id, bounty: 1000, town_id: winterhold.id)
...
kill_heimskr = Interaction.find_or_create_by(description: "You slice Savos Aren cleanly through the chest with your blade.", npc_id: heimskr.id, bounty: 1000, town_id: whiterun.id)
talk_heimskr = Interaction.find_or_create_by(description: "You have come! You have come to hear the word of Talos! The truth, child of Talos, is that the Dragon's children have come to purge the world in fire and righteousness! There are those who would silence the Dragon's truth... BUT NOT I! Not Heimskr! His word will be known! He has returned... Oh, how Talos has returned. Helgen has been purged in his light. Will Whiterun be next? The Imperials silence us because they fear us! They are cowards! Cowards and fools who have forgotten the truth! ... May Talos guide you ... ", npc_id: heimskr.id, town_id: whiterun.id)
kill_whiterun_jarl = Interaction.find_or_create_by(description: "You destroy Jarl Balgruuf the Greater's torso with a firebolt spell.", npc_id: whiterun_jarl.id, bounty: 1000, town_id: whiterun.id)
